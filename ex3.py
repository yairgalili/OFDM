...
def simulate_eigen_bf_qpsk(snr_db_values, Nrx, Ntx, num_symbols=1000000):
    ...
    for snr_db in snr_db_values:
        ...
        H = (
            np.random.randn(num_symbols, Nrx, Ntx)
            + 1j * np.random.randn(num_symbols, Nrx, Ntx)
        ) / np.sqrt(2)

        # Noise
        n = (
            np.random.randn(num_symbols, Nrx) + 1j * np.random.randn(num_symbols, Nrx)
        ) * np.sqrt(noise_var)
...
def simulate_stc_qpsk(snr_db_values, Nrx, num_symbols=1000000):
    ...
    for snr_db in snr_db_values:
        ...
        # Rayleigh fading (matrix form)
        h = (
            np.random.randn(Ntx, Nrx, num_symbols)
            + 1j * np.random.randn(Ntx, Nrx, num_symbols)
        ) / np.sqrt(2)

        # Noise
        n = (
            np.random.randn(T, Nrx, num_symbols)
            + 1j * np.random.randn(T, Nrx, num_symbols)
        ) * np.sqrt(noise_var)
